  ...
  <div>
    <div class="filter-container" style="text-align: right;">
      <el-button
        ...
        type="primary"
        icon="el-icon-edit"
        @click="openSelectStartTimeDialog"
      >录入数据</el-button>
      <el-button
        v-if="!showAddLine"
        class="filter-item"
    ...
    <el-table
      ...
      size="mini"
    >
      <el-table-column prop="title" label="项目" width="300">
        <template slot-scope="scope">
          <span :class="tableHeaderClass(scope.row.header)">{{ scope.row.title || '&nbsp;' }}</span>
        </template>
      </el-table-column>
      <el-table-column
        v-for="(yearData,index) in yearDataList"
        :key="'left'+index+yearData.id"
        :min-width="minHeaderWidth"
      >
    ...
    <div v-if="!showAddLine" class="add-another-area">
      <span class="another-area">
        <span class="table-header">{{ supplementaryData.year }}营业收入合计：</span>
        <el-input
          v-model="supplementaryData.operatingIncome"
          placeholder="请输入营业收入合计"
          suffix-icon="el-icon-date"
          :style="{'width':inputWidth}"
        />
      </span>
      <span class="another-area">
        <span class="table-header">{{ supplementaryData.year }}营业利润合计：</span>
        <el-input
          v-model="supplementaryData.operatingProfit"
          placeholder="请输入营业利润合计"
          suffix-icon="el-icon-date"
          :style="{'width':inputWidth}"
        />
      </span>
    ...
    <el-dialog title="选择录入年月" :visible.sync="selectDataVisible" width="240px">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addLine()">提交</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
import { incomeSheet } from '@/utils/table-title'
...
export default {
  ...
  methods: {
    ...
    // 生成日期一列对象
    createYearTemplate() {
      return Object.assign({}, this.singleYearTemplate, this.startData, {
        id: this.id++
      })
    },
    addLine: function() {
      if (
        this.startData.year === undefined ||
        this.startData.month === undefined
      ) {
        this.$notify({
          title: '提示',
          message: '请选择开始年月',
